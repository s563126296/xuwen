...
export default function CorridorElasticityCard({ delay = '0s' }: { delay?: string }) {
  ...
  return (
    <CollapsibleCard
      ...
      title="道路剩余容量"
      icon={<Gauge size={12} color="#4da6ff" />}
      summary={summary}
      delay={delay}
    >
        ...
        {corridorElasticity.map((item, i) => {
          ...
          return (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <span style={{ fontSize: 10, color: '#C9CDD4', width: 70, flexShrink: 0, textAlign: 'right' }}>{item.name}</span>
              ...
              <span style={{ fontFamily: 'DIN, sans-serif', fontWeight: 700, fontSize: 11, color, width: 36, textAlign: 'right' }}>{item.remainingPercent}%</span>
              <span style={{ fontSize: 10, color: '#A0A8B4', width: 55, textAlign: 'right' }}>+{item.remainingVehicles}辆</span>
            </div>
          );
        })}
      </div>
      {critical.length > 0 && (
        <div style={{ marginTop: 6, fontSize: 10, color: '#FF6B35', padding: '4px 8px', background: 'rgba(255, 107, 53, 0.1)', borderRadius: 4 }}>
